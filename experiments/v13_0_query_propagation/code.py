...
# ===== query-driven propagation =====
def propagate(node, nodes):
    targets = query_nodes(node, nodes)

    for n in targets:
        for i in range(len(node.essences)):
            src = random.choice(n.essences)
            sim = cosine_similarity(node.essences[i], src)

            if sim < 0.6:
                node.essences[i] += 0.05 * (src - node.essences[i])
            elif sim > 0.85:
                node.essences[i] -= 0.05 * (src - node.essences[i])
