...
def query_nodes(node, nodes):
    ...
    for n in candidates:
        ...
        novelty = 1.0 - sim
        score = 0.5 * n.trust + 0.5 * novelty
        scored.append((score, n))

    scored.sort(reverse=True, key=lambda x: x[0])
    return [n for _, n in scored[:QUERY_K]]

# ===== FIXED integration (diversity preserving) =====
def integrate(node, sources):
    if not sources:
        return

    for i in range(len(node.essences)):
        vec = np.zeros_like(node.essences[i])
        total_w = 0.0

        for src in sources:
            e = random.choice(src.essences)
            w = src.trust
            vec += w * e
            total_w += w
        ...
        # ★ FIX: reduce influence + add repulsion to prevent collapse
        sim = cosine_similarity(node.essences[i], vec)

        if sim < 0.7:
            node.essences[i] += 0.15 * (vec - node.essences[i])
        else:
            # repel if too similar
            node.essences[i] -= 0.05 * (vec - node.essences[i])
