...
def update_trust(nodes):
    ...
    for i, n in enumerate(nodes):
        ...
        anomaly_scale = 1.0 - ANOMALY_WEIGHT * (
            n.anomaly_score / (1 + n.anomaly_score)
        )
        ...
        n.trust = float(
            np.clip(new_trust, MIN_TRUST, MAX_TRUST)
        )
...
def update_essences(nodes):
    ...
    for node in nodes:
        ...
        for i in range(ESSENCE_SLOTS):
            ...
            src = random.choice(neighbors).essences[
...
print(
    "trust:",
    [round(n.trust, 3) for n in nodes]
)

print(
    "anomaly_score:",
    [round(n.anomaly_score, 3) for n in nodes]
)

print(
    "connections:",
    [list(n.connections) for n in nodes]
)
